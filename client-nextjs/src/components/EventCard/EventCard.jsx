...
const EventCard = ({event}) => {
	...
	return (
		<div  onClick={() => router.push(`/event/${event.id}`)} className="relative hover:shadow-xl transform transition duration-500 flex w-full flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-lg">
			
			
			<Link href={`/event/${event.id}`}>
				{/* <img fill className="lg:h-60  object-cover h-full w-full group-hover:scale-110 transition"  src={event.image} alt=''/> */}
			</Link>
			<div className="relative mx-4 mt-4 overflow-hidden rounded-xl bg-blue-gray-500 bg-clip-bordershadow-lg shadow-blue-gray-500/40">
				...
				<div className="text-center mb-5">
					<span className="badge  text-black h-10 rounded px-1 py-1 text-center text-xl font-bold cursor-pointer">{event.eventType}</span>
				</div>
				<div className="text-center mb-5">
					<p className="badge  text-black h-10 rounded px-1 py-1 text-center  font-bold cursor-pointer">{formatearFecha(event.date)}</p>
				</div>
